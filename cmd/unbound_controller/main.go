...
func main() {
	...
	zoneFile := flag.String("zoneFile", "/usr/local/etc/unbound/zones/zonefile", "Path to unbound zonefile")
...
`
	...
	mux.HandleFunc("/notify", func(w http.ResponseWriter, r *http.Request) {
		...
		_, _, err = core.RunCmd(&core.CmdOptions{
			Path: "/usr/local/sbin/unbound-checkconf",
		})

		if err != nil {
			log.Printf("somehow, the generated unbound conf is invalid: %v", err)
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		...
		_, _, err = core.RunCmd(&core.CmdOptions{
			...
			Args: []string{"/usr/sbin/service", "unbound", "reload"},
		})

		if err != nil {
			log.Printf("failed to reload unbound service: %v", err)
			w.WriteHeader(http.StatusInternalServerError)
			return
		}

		log.Printf("new unbound conf applied: %v", event)
